...
场景 ID：`materials_screen`。本目录中的 `SPEC` 是注册表的唯一参数来源，`execute` 是实际执行入口。

## 独立运行

```bash
python -m agrel_public.scenarios.materials_screen --device cuda:0 --out runs/materials_screen-new
```
