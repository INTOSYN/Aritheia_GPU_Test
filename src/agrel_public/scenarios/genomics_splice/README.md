...
场景 ID：`genomics_splice`。本目录中的 `SPEC` 是注册表的唯一参数来源，`execute` 是实际执行入口。

## 独立运行

```bash
python -m agrel_public.scenarios.genomics_splice --device cuda:0 --out runs/genomics_splice-new
```
