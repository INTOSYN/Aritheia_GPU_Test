...
场景 ID：`medical_ultrasound`。本目录中的 `SPEC` 是注册表的唯一参数来源，`execute` 是实际执行入口。

## 独立运行

```bash
python -m agrel_public.scenarios.medical_ultrasound --device cuda:0 --out runs/medical_ultrasound-new
```
