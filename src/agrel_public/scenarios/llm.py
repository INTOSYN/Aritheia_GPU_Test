...
"""Small pinned real LLM for the two language scenarios (ported from v0.3.0).
Weights come from the 'language' optional pack; no remote API, no random
...
def model_dir() -> Path:
    p = assets.locate(f"{assets.LLM_DIR}/manifest.json")
    if p is None:
...
def verify_model(path: Path) -> str:
    ...
    for name, expected in manifest["sha256"].items():
        ...
        if not file.exists() or sha256_file(file) != expected:
            raise ValueError(f"LLM file integrity mismatch: {name}")
    return sha256_file(path / "manifest.json")
...
def evaluate(name: str, device: str, precision: str, batch_size: int, context_size: str, max_items=None, max_length: int = 4096):
    ...
    model = AutoModelForCausalLM.from_pretrained(path, local_files_only=True, trust_remote_code=False,
                                                 torch_dtype=dtype_for(precision), attn_implementation="eager").to(device).eval()
    ...
    for r in records:
        text = tok.apply_chat_template([{"role": "user", "content": prompt_text(r, context_size)}], tokenize=False, add_generation_prompt=True)
    ...
    actions = [{"id": r["id"], "selected": r["options"][int(j)], "choice": "ABCD"[int(j)], "executed": False}
    ...
    detail = {"task": "classification", "model_id": MODEL_ID, "model_revision": REVISION, "context": context_size,
              ...
              "sampling": False, "kv_cache": False, "attention": "eager",
