...
场景 ID：`molecule_neighbors`。本目录中的 `SPEC` 是注册表的唯一参数来源，`execute` 是实际执行入口。

## 独立运行

```bash
python -m agrel_public.scenarios.molecule_neighbors --device cuda:0 --out runs/molecule_neighbors-new
```
