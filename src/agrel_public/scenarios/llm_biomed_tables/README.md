...
场景 ID：`llm_biomed_tables`。本目录中的 `SPEC` 是注册表的唯一参数来源，`execute` 是实际执行入口。

## 独立运行

```bash
python -m agrel_public.scenarios.llm_biomed_tables --device cuda:0 --out runs/llm_biomed_tables-new
```
